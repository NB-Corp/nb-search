...
export function loadConfiguration(env: NodeJS.ProcessEnv = process.env, transport: JsonTransport = new FetchJsonTransport(), options: LoadConfigurationOptions = {}): AppConfiguration {
  ...
  for (const [id, lane] of Object.entries(resolved.config.lanes)) {
    ...
    const ready = issues.length === 0; const builtIn = descriptor !== undefined && registry.isBuiltIn(instance.provider_id); const executionModes: QueryExecution[] = queryOperation === undefined || !ready ? [] : builtIn && queryOperation.built_in_async ? ['sync', 'async'] : ['sync'];
