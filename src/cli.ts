...
async function runResearch(runtime: NbSearchRuntime, argv: readonly string[]): Promise<unknown> {
  ...
  if (action === 'read') {
    ...
    if (artifact !== undefined && artifact !== 'summary' && artifact !== 'report' && artifact !== 'sources' && artifact !== 'capabilities' && artifact !== 'multi_agent_research') throw invalidInput('--artifact must be summary, report, sources, capabilities, or multi_agent_research.');
