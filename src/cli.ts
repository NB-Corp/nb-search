...
import type { JobState, ResearchArtifact } from './types.ts';
...
async function runSearch(runtime: NbSearchRuntime, argv: readonly string[]): Promise<unknown> {
  const parsed = parse(argv, new Set(['--max-results', '--timeout-ms']));
...
async function runResearch(runtime: NbSearchRuntime, argv: readonly string[]): Promise<unknown> {
  const action = argv[0];
  if (action === 'start') {
    const parsed = parse(argv.slice(1), new Set(['--max-sources', '--max-duration-ms', '--idempotency-key']));
