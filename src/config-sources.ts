...
export function defaultConfiguration(home: string): CanonicalConfig {
  return {
    ...
    defaults: {}, presets: {},
    execution: { max_provider_calls: 64, max_concurrency: 8, retry_count: 1, search_timeout_ms: 30_000, fetch_timeout_ms: 60_000, max_inline_bytes: 64 * 1024, fetch: { max_response_bytes: 2 * 1024 * 1024, max_content_chars: 200_000, max_redirects: 5 } },
  };
}
export function resolveConfiguration(options: ResolveConfigurationOptions = {}): ResolvedConfiguration {
  ...
  const fingerprint = stableFingerprint(config); return { config, config_revision: `config-3-${fingerprint.slice(0, 16)}`, config_fingerprint: fingerprint, secret_bindings: secrets, canonical_path: canonicalPath };
}
function environmentPatch(env: NodeJS.ProcessEnv): CanonicalConfigPatch {
  ...
  set('exa.default', 'base_url', nonempty(env['NB_SEARCH_EXA_BASE_URL'])); set('tavily.default', 'base_url', nonempty(env['NB_SEARCH_TAVILY_BASE_URL'])); set('grok.default', 'base_url', nonempty(env['NB_SEARCH_GROK_BASE_URL'])); set('grok-multi-agent.default', 'base_url', nonempty(env['NB_SEARCH_GROK_MULTI_AGENT_BASE_URL'])); set('search-gateway.default', 'base_url', nonempty(env['NB_SEARCH_GATEWAY_BASE_URL']));
