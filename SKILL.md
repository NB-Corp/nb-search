...
- 模型负责判断查什么、是否拆分 query、选择哪个 lane；runtime 只执行显式选择或本机默认。
- 不要期待自动 fallback。显式 lane 不可用时，根据错误决定是否另发一次调用。
- `execution` 只改变交付方式，不改变 query、lane 或 plan。
...
- 成功内容位于 `documents`；失败信息位于 `lane_outcomes` 和 `hints`，不会创建失败 document 占位。
