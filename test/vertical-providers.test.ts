...
describe('Context7 docs adapter', () => {
  it('resolves a library then returns stable typed documentation context', async () => {
    ...
    expect(new URL(transport.requests[0]!.url).searchParams.get('query')).toBe('query');
  ...
  it('preserves timeout aborts and is ready without a key', async () => {
    await expect(timeout((signal) => new Context7DocsProvider({ transport: new QueueTransport([]) }).execute(queryRequest(signal)))).rejects.toMatchObject({ code: 'DEADLINE_EXCEEDED' });
...
describe('Zhipu search adapter', () => {
  it('builds the search_std request and normalizes results', async () => {
    ...
    expect(transport.requests[0]).toMatchObject({ url: 'https://proxy.test/api/paas/v4/web_search', method: 'POST', headers: { Authorization: 'Bearer zhipu-secret', Accept: 'application/json', 'Content-Type': 'application/json' }, body: { search_query: 'query', search_engine: 'search_std' } });
  });
  ...
  it('preserves timeout aborts and requires a credential for readiness', async () => {
...
describe('GitHub repository search adapter', () => {
  ...
  it('preserves timeout aborts and reports unauthenticated rate-limit risk while ready', async () => {
...
function searchRequest(signal = new AbortController().signal) { return { query: 'query', limit: 3, signal }; }
