...
import { mockRegistration } from './helpers.ts';
...
function request() { return { url: 'https://example.com/page', signal, max_response_bytes: 4096, max_content_chars: 1000, max_redirects: 5 }; }
function availability(value: Awaited<ReturnType<ReturnType<typeof createRuntimeComposition>['runtime']['capabilities']>>): Record<string, string> { return Object.fromEntries(value.fetch.lanes.map((lane) => [lane.id, lane.availability])); }
