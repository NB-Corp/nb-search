...
describe('fetch serial lane chain', () => {
  ...
  it('uses an explicit lane without touching the configured chain', async () => {
    ...
    const value = await app.runtime.fetch({ action: 'run', source: { kind: 'url', url: 'https://example.com' }, pipeline: 'c.fetch' }); expect(value.documents[0]?.source_lane).toBe('c.fetch'); expect(value.selection).toEqual({ source: 'pipeline', pipeline: 'c.fetch' }); expect(calls).toEqual({ a: 0, b: 0, c: 1 });
  });
...
function document(url: string, content: string, representation: 'markdown' | 'text' = 'markdown'): FetchProviderResult { return { url, final_url: url, content, content_type: 'text/plain', media_type: 'text/plain', representation, format: 'text', byte_length: Buffer.byteLength(content), truncated: false, warnings: [] }; }
